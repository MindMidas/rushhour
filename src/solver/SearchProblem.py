# Generic search framework used by the Rush Hour solver.
...
class SearchProblem:
  ...
  discovered = set(); # class variable that holds unique states
                      # generated by a search, including frontier states
  ...
  def add_discovered_state(state, depth):
    """
    Track one generated state once, even if it has not been expanded yet.
    """;

    state_repr = repr(state.state);
    if state_repr in SearchProblem.discovered:
      return False;

    SearchProblem.discovered.add(state_repr);
    if depth not in SearchProblem.unique_dictionary:
      SearchProblem.unique_dictionary[depth] = 0;
    SearchProblem.unique_dictionary[depth] += 1;
    return True;
  ...
  def dfs( self, max_depth=None, continue_search=None ):
    ...
    SearchProblem.add_discovered_state(self, 0);
    ...
    while stack:
      ...
      for action in reversed(curr_state.edges()):
        ...
        SearchProblem.add_discovered_state(action.destination, SearchProblem.depth + 1);
    ...
    while queue:
      ...
      for action in curr_state.edges():
        ...
        if repr(action.destination.state) not in path:
            ...
            next_depth = SearchProblem.depth + 1;
            SearchProblem.add_discovered_state(action.destination, next_depth);

            # append new state to queue
            queue.append((action.destination, action_path, next_depth));
  
  def best_fs(self, heuristic=None, max_depth=None, continue_search=None):
    """
    ...
    while queue:
      ...
      for action in curr_state.edges():
        ...
        if repr(action.destination.state) not in path:
          ...
          next_depth = SearchProblem.depth + 1;
          SearchProblem.add_discovered_state(action.destination, next_depth);
          SearchProblem.admissability.append((heuristic_val, next_depth));
          heapq.heappush(queue, (heuristic_val + next_depth, next_depth, action.destination, action_path));
